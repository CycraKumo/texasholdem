def get_hand_rank_dict():
    ...
    for hand, rank in rankings.items():
        play2.append({hand: rank[0]})
        play3.append({hand: rank[1]})
        play4.append({hand: rank[2]})
        play5.append({hand: rank[3]})
        play6.append({hand: rank[4]})
        play7.append({hand: rank[5]})
        play8.append({hand: rank[6]})
        play9.append({hand: rank[7]})
        play10.append({hand: rank[8]})

    play2 = sorted(play2, key=lambda x: list(x.values())[0])
    play3 = sorted(play3, key=lambda x: list(x.values())[0])
    play4 = sorted(play4, key=lambda x: list(x.values())[0])
    play5 = sorted(play5, key=lambda x: list(x.values())[0])
    play6 = sorted(play6, key=lambda x: list(x.values())[0])
    play7 = sorted(play7, key=lambda x: list(x.values())[0])
    play8 = sorted(play8, key=lambda x: list(x.values())[0])
    play9 = sorted(play9, key=lambda x: list(x.values())[0])
    play10 = sorted(play10, key=lambda x: list(x.values())[0])

    play2 = [{list(hand.keys())[0]: str(index + 1).zfill(3)} for index, hand in enumerate(play2)]
    play3 = [{list(hand.keys())[0]: str(index + 1).zfill(3)} for index, hand in enumerate(play3)]
    play4 = [{list(hand.keys())[0]: str(index + 1).zfill(3)} for index, hand in enumerate(play4)]
    play5 = [{list(hand.keys())[0]: str(index + 1).zfill(3)} for index, hand in enumerate(play5)]
    play6 = [{list(hand.keys())[0]: str(index + 1).zfill(3)} for index, hand in enumerate(play6)]
    play7 = [{list(hand.keys())[0]: str(index + 1).zfill(3)} for index, hand in enumerate(play7)]
    play8 = [{list(hand.keys())[0]: str(index + 1).zfill(3)} for index, hand in enumerate(play8)]
    play9 = [{list(hand.keys())[0]: str(index + 1).zfill(3)} for index, hand in enumerate(play9)]
    play10 = [{list(hand.keys())[0]: str(index + 1).zfill(3)} for index, hand in enumerate(play10)]

    play_lists = [play3, play4, play5, play6, play7, play8, play9, play10]

    merged_dict = {}

    # まず基本となるラベルを作る
    for hand_rank in play2:
        merged_dict[list(hand_rank.keys())[0]] = list(hand_rank.values())[0]
...
def get_hand_rank(hand, num_players, rankings):
    # すべてのハンドのランキング情報

    # プレイヤー数に応じたインデックスを取得
    index = (num_players - 2) * 3

    # 指定されたハンドのランキングを取得
    rank_str = rankings[hand][index:index+3]

    return int(rank_str)


merged_dict = get_hand_rank_dict()
